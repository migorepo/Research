...
// Updated ver of Peter torr's MemoryDiagnosticsHelper (Wp8)
...
namespace Wp8.Framework.Utils.Common
{
    ...
    public static class MemoryDiagnosticsHelper
    {
        ...
        static bool _alreadyFailedPeak; // to avoid endless Asserts
        ...
        public static void Start(TimeSpan timespan, bool forceGc)
        {
            ...
            _maxMemory = GetMemoryUsageLimit() * 1024 * 1024;
        ...
        /// <summary>
        /// Gets the  memory usage limit, in bytes. Returns zero in non-debug mode
        /// </summary>
        /// <returns>Peak memory usage</returns>
        ...
        {
            if (_alreadyFailedPeak)
                return;

            long peak = GetPeakMemoryUsage();
            if (peak >= _maxMemory)
            {
                _alreadyFailedPeak = true;
                Checkpoint("*MEMORY USAGE FAIL*");
                _peakMemoryBlock.Text = "FAIL!";
                _peakMemoryBlock.Foreground = new SolidColorBrush(Colors.Red);
        ...
        {
            double percent = mem / (double)_maxMemory;
            if (percent <= 0.75)
                return 0;
